...
/**
 * Build a QBIndexInput from an AthleteMetrics object.
 * All four extended fields (footwork, poise, fieldVision, clutchFactor)
 * are now tracked directly on AthleteMetrics, so no estimation is needed.
 */
export function metricsToQBIndexInput(m: AthleteMetrics): QBIndexInput {
  return {
    velocity: m.velocity,
